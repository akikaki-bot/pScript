...
export function lex(input: string): Token[] {
    ...
    while (i < input.length) {
        ...
        if (isIdStart(ch) || (ch === '.' && isIdStart(input[i + 1] || ''))) {
            let j = i;
            if (ch === '.') j++;
            while (j < input.length) {
                const c = input[j];
                if (isId(c)) { j++; continue; }
                if (c === '.' && isIdStart(input[j + 1] || '')) { j++; continue; }
                if (c === '[') {
                    let depth = 1;
                    j++;
                    while (j < input.length && depth > 0) {
                        ...
                        if (input[j] === '[') { depth++; j++; continue; }
                        if (input[j] === ']') { depth--; j++; continue; }
                        j++;
                    }
                    if (depth !== 0) throw new TypeError('Unterminated bracket in identifier at ', "", i);
                    continue;
                }
                break;
            }
